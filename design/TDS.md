...
| FR-S06 | Must | Wind speed shall be computed in the millisecond domain as `v[0.1 m/s] = (count × C_scaled × 10) / window_ms`, where C_scaled is the calibration factor in units of 0.001 m/rotation (FR-S25) and window_ms is the duration of the window in which the pulses were counted (the value 40002 had when that window opened). Results exceeding 65535 shall be clamped to 65535. | With 40003 = 10000 ms and a pulse generator at known frequency f: 30002 equals round(f × C × 10) within ±(one pulse-count quantum for the window + 1 LSB) — i.e. ±2 LSB at a 10 s window. With C_scaled = 980, 40003 = 500 and a 10 Hz input: 30002 reads 98 ±2 LSB (no division-by-zero or truncation at sub-second windows). |
...
| FR-S25 | Must | The calibration factor C shall be a compile-time constant set via a pre-processor define, integer fixed-point in units of 0.001 m/rotation, valid range 1–6553 enforced by a static build-time assert, default 980 (the known r = 0.07 m / η = 0.45 rotor), documented in the source tree. No holding register shall map to C. | Two builds with C_scaled = 980 and 1100, driven at the same fixed pulse rate, report 30002 in the ratio 980:1100 ±1 LSB. A build with C_scaled = 0 or 6554 fails to compile. |
